...
function ThankYou({ riskResult, formData, questionnaireData }) {
  ...
  const handleDownloadPdf = () => {
    // ... (Existing, working PDF generation logic - UNCHANGED)
    if (!formData) {
      alert("Form data is not available to generate a PDF.");
      return;
    }
    ...
    if (typeof doc.autoTable !== 'function') {
        console.error("jsPDF autoTable plugin is not loaded correctly. Check the import statement.");
        alert("Could not generate PDF. The autoTable plugin is missing.");
        return;
    }
    const pageWidth = doc.internal.pageSize.getWidth();
    const themeColor = [98, 0, 238];
    ...
    formStructure.forEach(section => {
        ...
        const findAnsweredQuestions = (questions, parentNumber) => {
            questions.forEach((qConfig, index) => {
                ...
                if (answer !== undefined && answer !== null && answer !== '' && (!Array.isArray(answer) || answer.length > 0)) {
                    questionsToRender.push({ questionObject: { title: `Question ${displayNumber}`, text: questionnaireData[qConfig.key]?.question, }, answer: Array.isArray(answer) ? answer.join(', ') : answer.toString() });
                }
                if (qConfig.subQuestions && qConfig.condition && formData[qConfig.condition.key] === qConfig.condition.value) {
                    findAnsweredQuestions(qConfig.subQuestions, displayNumber);
                }
        ...
        if (questionsToRender.length > 0) {
            body.push([{ content: section.title, colSpan: 2, styles: { fontStyle: 'bold', fontSize: 14, textColor: themeColor, fillColor: [245, 245, 245], halign: 'left' } }]);
            questionsToRender.forEach(qa => { body.push([qa.questionObject, qa.answer]); });
        }
    });
    autoTable(doc, {
        startY: 30,
        head: riskResult !== null ? [[{ content: `Your Estimated Lifetime Risk Score: ${score}`, styles: { halign: 'center', valign: 'middle', fontStyle: 'bold', fontSize: 16, textColor: themeColor, fillColor: [240, 230, 255], minCellHeight: 22, lineWidth: 0.2, lineColor: [200, 180, 255] } }]] : [],
        body: body, theme: 'grid', didDrawPage: pageHooks.didDrawPage,
        styles: { cellPadding: 3.5, fontSize: 10, valign: 'middle', lineWidth: 0.1, lineColor: [230, 230, 230], },
        columnStyles: { 0: { cellWidth: 85, fontStyle: 'normal' }, 1: { cellWidth: 'auto', textColor: [40, 40, 40] } },
        didParseCell: (data) => {
            if (data.column.index === 0 && typeof data.cell.raw === 'object' && data.cell.raw !== null) {
                data.cell.text = ''; doc.setFont('helvetica', 'bold'); doc.setFontSize(10); doc.setTextColor(40, 40, 40);
                doc.text(data.cell.raw.title, data.cell.x + data.cell.padding('left'), data.cell.y + data.cell.padding('top'));
                doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(120, 120, 120);
                doc.text(data.cell.raw.text, data.cell.x + data.cell.padding('left'), data.cell.y + data.cell.padding('top') + 6);
            }
        },
